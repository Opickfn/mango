...
export const NavActions = ({ user }: { user: any }) => {
  ...
  const rawRole = user?.roles?.[0] || t("loading");
  const displayRole = rawRole.replace('_', ' ');
  const displayName = user?.name || t("user_fallback");
  const avatarUrl = user?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=dbeafe&color=1e477e&bold=true`;
  const isDuplicate = displayName.toLowerCase() === displayRole.toLowerCase();
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        >
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full overflow-hidden border border-border">
