...
import { InitialsAvatar } from "@/src/components/ui/dashboard/AdminDataView";
import {
  AdminTable,
  AdminTableBody,
  AdminTableCell,
      ...
      <div className="space-y-6">
        ...
        {organization && (
          <div className="flex items-center gap-3 p-4 bg-primary/5 border border-primary/10 rounded-xl">
            ...
            <div>
              <p className="text-xs font-medium text-muted-foreground">Otoritas unit</p>
              <h2 className="text-sm font-semibold text-foreground mt-0.5">{organization.name}</h2>
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <AdminTable>
              <AdminTableHeader>
                <AdminTableRow>
                  <AdminTableHeadCell>Informasi pendaftar</AdminTableHeadCell>
                  <AdminTableHeadCell>Waktu mendaftar</AdminTableHeadCell>
                  <AdminTableHeadCell align="right">Opsi otorisasi</AdminTableHeadCell>
                </AdminTableRow>
              </AdminTableHeader>
              <AdminTableBody>
                {members.map((user) => (
                  <AdminTableRow key={user.id}>
                    ...
                    <AdminTableCell>
                      <span className="text-sm text-muted-foreground">
                        {user.joined_at ? new Date(user.joined_at).toLocaleString() : "Menunggu"}
                      </span>
                    </AdminTableCell>
