                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="space-y-1.5">
                                        <Label className="text-xs font-bold text-muted-foreground ml-1">{t("instagram_label")}</Label>
                                        <div className="relative">
                                            <Camera className="absolute left-3 top-3 text-muted-foreground/50" size={16} />
                                            <Input {...register("instagram")} className={inputClass + " pl-10 text-xs"} placeholder="@username" disabled={isSubmitting} />
                                        </div>
                                    </div>
                                    <div className="space-y-1.5">
                                        <Label className="text-xs font-bold text-muted-foreground ml-1">{t("facebook_label")}</Label>
                                        <div className="relative">
                                            <Share2 className="absolute left-3 top-3 text-muted-foreground/50" size={16} />
                                            <Input {...register("facebook")} className={inputClass + " pl-10 text-xs"} placeholder="Username" disabled={isSubmitting} />
                                        </div>
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="space-y-1.5">
                                        <Label className="text-xs font-bold text-muted-foreground ml-1">{t("shopee_label")}</Label>
                                        <div className="relative">
                                            <ShoppingBag className="absolute left-3 top-3 text-muted-foreground/50" size={16} />
                                            <Input {...register("shopee")} className={inputClass + " pl-10 text-[10px]"} placeholder="Link Shopee" disabled={isSubmitting} />
                                        </div>
                                    </div>
                                    <div className="space-y-1.5">
                                        <Label className="text-xs font-bold text-muted-foreground ml-1">{t("tokopedia_label")}</Label>
                                        <div className="relative">
                                            <ShoppingBag className="absolute left-3 top-3 text-muted-foreground/50" size={16} />
                                            <Input {...register("tokopedia")} className={inputClass + " pl-10 text-[10px]"} placeholder="Link Tokopedia" disabled={isSubmitting} />
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="space-y-6 py-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <FormItem label={t("established_year_label")} error={errors.established_year?.message}>
                            <Input type="number" {...register("established_year")} className={inputClass} placeholder="YYYY" disabled={isSubmitting} />
                        </FormItem>
                    </div>
        ...
        <div className="lg:col-span-4">
            <SectionCard title={t("legal_documents") || "Data legalitas"} icon={ShieldCheck} className="h-full rounded-xl border-border/50 shadow-none">
                <div className="space-y-5">
                    ...
                    <FormItem label={t("owner_name")} error={errors.owner_name?.message}>
                        <Input {...register("owner_name")} className={inputClass} placeholder="Nama Pemilik" disabled={isSubmitting} />
                    </FormItem>
                    <div className="grid grid-cols-2 gap-4">
                        <FormItem label="NIK (Terkunci)" error={errors.nik?.message}>
                            <Input {...register("nik")} disabled={true} className="h-11 rounded-lg bg-muted/40 border-none font-mono text-xs opacity-60" />
                        </FormItem>
                        <FormItem label={t("npwp")} error={errors.npwp?.message}>
                            <Input {...register("npwp")} className={inputClass + " font-mono text-xs"} placeholder="NPWP" disabled={isSubmitting} />
                        </FormItem>
                    </div>
