                    ...
                    <div className="bg-primary text-primary-foreground rounded-xl p-8 shadow-sm relative overflow-hidden flex-1 flex flex-col justify-between border border-primary/20">
                        ...
                        <div className="relative z-10 space-y-10">
                            <div className="flex justify-between items-start">
                                <div className="w-20 h-20 rounded-2xl shadow-xl overflow-hidden border-2 border-white/20 bg-white/10 flex-shrink-0">
                                    {umkm.logo_url && !umkm.logo_url.includes('placeholders') ? (
                                        ...
                                        <div className="w-full h-full bg-white flex items-center justify-center text-primary font-black text-2xl uppercase">
                                            {umkm.name?.charAt(0) || "M"}
                                        </div>
                                    )}
                                </div>
                                <BadgeCheck className="text-white opacity-40" size={32} strokeWidth={1.5} />
                            </div>
                            ...
                            <div className="space-y-4 pt-6 border-t border-white/10 mt-auto">
                                <div>
                                    ...
                                    <p className="text-sm font-bold truncate">{umkm.name}</p>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4 pb-2">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-sm font-bold">{umkm.established_year}</p>
                                    </div>
                                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <SectionCard title={t("digital_presence")} icon={Globe} className="rounded-xl border-border/50">
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <SocialLink icon={Camera} label={t("instagram_label")} value={umkm.instagram ? `@${umkm.instagram.replace('@','')}` : null} isLink url={`https://instagram.com/${umkm.instagram?.replace('@','')}`} />
                                    <SocialLink icon={Share2} label={t("facebook_label")} value={umkm.facebook} isLink />
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    <SocialLink icon={ShoppingBag} label={t("shopee_label")} value={umkm.shopee ? "Lihat Shopee" : null} isLink url={umkm.shopee} />
                                    <SocialLink icon={ShoppingBag} label={t("tokopedia_label")} value={umkm.tokopedia ? "Lihat Toko" : null} isLink url={umkm.tokopedia} />
                                </div>
                            </div>
                        </SectionCard>
                        ...
                        <SectionCard title={t("legal_documents")} icon={FileText} className="rounded-xl border-border/50">
                            <div className="space-y-3">
                                ...
                                <Button variant="outline" className="w-full justify-between rounded-xl h-11 text-xs font-bold border-border/50 hover:bg-muted/30" onClick={() => umkm.ktp_url && window.open(umkm.ktp_url, '_blank')}>
                                    <span className="flex items-center gap-2"><UserIcon size={14} className="text-primary" strokeWidth={1.5} /> KTP Pemilik</span>
                                    <Download size={14} strokeWidth={1.5} />
                                </Button>
                            </div>
                        </SectionCard>
