...
class UmkmService
{
    ...
    public function create(User $user, array $data): Umkm
    {
        return DB::transaction(function () use ($user, $data) {
            ...
            $ktpFile = $data['ktp_file'] ?? null;

            unset($data['logo'], $data['nib_file'], $data['ktp_file']);
            ...
            if ($ktpFile instanceof UploadedFile) {
                $umkm->addMedia($ktpFile)->toMediaCollection('ktp_documents');
            }
    ...
    public function update(Umkm $umkm, array $data): Umkm
    {
        return DB::transaction(function () use ($umkm, $data) {
            ...
            if (isset($data['ktp_file']) && $data['ktp_file'] instanceof UploadedFile) {
                $umkm->addMedia($data['ktp_file'])->toMediaCollection('ktp_documents');
            }
